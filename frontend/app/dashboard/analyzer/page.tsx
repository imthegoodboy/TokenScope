...
function getSuggestions(analysis: AnalyzeResult & { score: number; breakdown: { clarity: number; conciseness: number; specificity: number; efficiency: number } }): Array<{ icon: LucideIcon; title: string; desc: string; impact: "high" | "medium" | "low" }> {
  ...
  if (analysis.breakdown.clarity < 70) {
    suggestions.push({ icon: MessageSquare, title: "Improve clarity", desc: "Be more explicit about the task. Use step-by-step instructions.", impact: "high" as const });
  }
  if (analysis.breakdown.specificity < 70) {
    suggestions.push({ icon: Target, title: "Add specificity", desc: "Include examples, format requirements, or constraints to guide the model.", impact: "medium" as const });
  }
  if (!analysis.token_scores.some((t) => t.token.includes("format") || t.token.includes("output"))) {
    suggestions.push({ icon: Sparkles, title: "Specify output format", desc: "Tell the model how to structure its response (JSON, bullet points, etc.).", impact: "low" as const });
  }
  return suggestions;
}
...
export default function AnalyzerPage() {
  ...
  const [analysis, setAnalysis] = useState<(AnalyzeResult & { score: number; breakdown: { clarity: number; conciseness: number; specificity: number; efficiency: number } }) | null>(null);
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await request<AnalyzeResult>("/api/v1/analyze/prompt", {
        method: "POST",
      ...
      const clarity = Math.min(100, Math.floor(50 + avgScore * 50 + (words.length < 100 ? 20 : 0)));
      ...
      setAnalysis({ ...result, score: overallScore, breakdown: { clarity, conciseness, specificity, efficiency } });
      setOptimized(null);
  ...
  const handleOptimize = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await request<OptimizeResult>("/api/v1/analyze/optimize", {
        method: "POST",
        body: JSON.stringify({ prompt, model, provider }),
      });
      setOptimized(result);
  ...
  return (
    <div>
      <Header title="Prompt Analyzer" description="Analyze token usage, get TF-IDF insights, and optimize prompts" />
      ...
      <div className="px-8 py-6">
        ...
        <div className="flex flex-wrap items-end gap-4 mb-6">
          <div className="flex-1 min-w-40">
            ...
            <Select options={PROVIDERS} value={provider}
              onChange={(e) => { setProvider(e.target.value); setModel(MODELS[e.target.value][0].value); setAnalysis(null); setOptimized(null); }} />
          </div>
          <div className="flex-1 min-w-40">
            <label className="block text-xs font-medium opacity-60 mb-1.5">Model</label>
            <Select options={MODELS[provider]} value={model}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="card">
              <label className="block text-xs font-semibold uppercase tracking-wider opacity-60 mb-3">Your Prompt</label>
              <textarea className="w-full h-48 p-4 rounded-xl border border-black-border bg-bg text-sm font-mono resize-none focus:outline-none focus:ring-2 focus:ring-jaffa/40 transition-all code-scroll"
                placeholder="Enter your AI prompt here to analyze token usage and optimization..."
                value={prompt}
              ...
              {error && (
                <div className="mt-3 p-2.5 rounded-lg bg-danger/10 border border-danger/20 text-xs text-danger">{error}</div>
              )}
            </div>
              ...
              <div className="card animate-fade-in">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs opacity-60 mt-0.5">Warm orange = high impact · Gray = low impact</p>
                  </div>
                  <div className="flex gap-3 text-xs">
                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-jaffa inline-block" /> High</span>
                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-jaffa/50 inline-block" /> Medium</span>
                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-black-border inline-block" /> Low</span>
                  </div>
                </div>
            ...
            {activeTab === "improve" && optimized && (
              <div className="card animate-slide-up border-jaffa/30">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <Badge className="bg-green-bg text-green border-green/20">Saved {optimized.saved_tokens} tokens</Badge>
                </div>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <span className="text-xs opacity-60">{optimized.optimized_tokens} tokens (was {optimized.original_tokens})</span>
                </div>
              </div>
            )}
          </div>
            ...
            {analysis ? (
              <>
                  ...
                  <div className="space-y-2.5">
                    {[
                      ...
                      <div key={item.label} className="flex justify-between items-center py-2 border-b border-black-border last:border-0">
                        ...
                        <span className={`font-mono font-medium ${item.color || ""} ${item.bold ? "font-bold" : ""}`}>{item.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="card">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {analysis.top_important.slice(0, 15).map((t, i) => (
                        <span key={i}
                          className={
                            t.importance === "very_high" ? "token-very-high" :
                            t.importance === "high" ? "token-high" :
                            t.importance === "medium" ? "token-medium" : "token-low"
                          }
                  ...
                  return (
                    <div className="card">
                      <h3 className="font-semibold mb-3 flex items-center gap-2">
                        <Lightbulb size={14} className="text-jaffa" /> Suggestions
                      </h3>
                ...
                <Button className="w-full"
                  ...
                  {activeTab === "analyze" ? (
                    optimizing ? <><RefreshCw size={14} className="animate-spin" /> Optimizing...</> : <><Sparkles size={14} /> Get Optimized Version</>
